...
// SAFETY: FontIDs contains only imgui::FontId values (Copy indices into the
// font atlas). The atlas is initialized once and lives for the overlay
// lifetime. FontId is just an index and carries no thread-affinity.
...
impl PracticeTool {
    ...
    fn render_closed(&mut self, ui: &imgui::Ui) {
        ...
        ui.window("##msg_window")
            ...
            .build(|| {
                ...
                self.render_indicators_popup(ui);
                ...
                self.render_help_popup(ui);
                self.render_update_popup(ui);
    ...
    fn render_indicators_popup(&mut self, ui: &imgui::Ui) {
        ui.modal_popup_config("##indicators_window")
            ...
            .build(|| {
                ...
                ui.text(
                    "You can toggle indicators here, as\nwell as reset the frame counter.\n\nKeep \
                     in mind that the available\nindicators and order of them depend\non your \
                     config file.",
                );
                ui.separator();
                ...
                for indicator in &mut self.settings.indicators {
                    ...
                    if let IndicatorType::FrameCount = indicator.indicator {
                        ...
                        let btn_reset_width =
                            ui.calc_text_size(btn_reset_label)[0] + style.frame_padding[0] * 2.0;
                    ...
                    if let IndicatorType::PositionDistance = indicator.indicator {
                        ...
                        let btn_xyz_width =
                            ui.calc_text_size(btn_xyz_label)[0] + style.frame_padding[0] * 2.0;
    ...
    fn render_help_popup(&mut self, ui: &imgui::Ui) {
        ui.modal_popup_config("##help_window")
            ...
            .build(|| {
                self.pointers.cursor_show.set(true);
                ui.text(formatcp!("Elden Ring Practice Tool v{}.{}.{}", MAJOR, MINOR, PATCH));
                ui.separator();
                ui.text(format!(
    ...
    fn render_update_popup(&mut self, ui: &imgui::Ui) {
        ui.modal_popup_config("##update").resizable(false).movable(false).title_bar(false).build(
            || {
                self.pointers.cursor_show.set(true);
            ...
            match indicator.indicator {
                ...
                IndicatorType::Position => {
                    if let (Some([x, y, z, _a1, _a2]), Some(m)) = (
                        ...
                        write!(self.position_bufs[0], "m{a:02x}_{b:02x}_{r:02x}_{s:02x}").ok();
                        ...
                        ui.same_line();
                        ui.text_colored([0.7048, 0.1228, 0.1734, 1.], &self.position_bufs[1]);
                        ui.same_line();
                        ui.text_colored([0.1161, 0.5327, 0.3512, 1.], &self.position_bufs[2]);
                        ui.same_line();
                        ui.text_colored([0.1445, 0.2852, 0.5703, 1.], &self.position_bufs[3]);
                    }
                },
                ...
                IndicatorType::PositionDistance => {
                    if let Some([x, y, z, _a1, _a2]) = self.pointers.global_position.read() {
                        ...
                        write!(
                            ...
                            "Distance: [XYZ] {position_dist_xyz:.4} | [XZ] {position_dist_xz:.4} \
                             | [Y] {position_dist_y:.4}"
                        )
                        .ok();
